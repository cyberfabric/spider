...
- [x] `p1` - **ID**: `spd-spaider-adr-template-centric-architecture-v1`
...
<!-- spd:context -->
**Context**:
The current Spaider architecture has grown organically with many separate workflow files (`prd.md`, `design.md`, `specs.md`, `spec.md`, `adr.md`, `code.md` plus their `-validate` variants) and content requirements scattered across `requirements/*-content.md` files. This creates several problems:
...
4. **No content versioning**: When an Spaider ID's content changes, references to that ID cannot detect the change, leading to stale cross-references
5. **Naming confusion**: "Spaider" has evolved beyond "Spec-Driven Design" and needs a name that reflects its broader scope as a documentation and development framework

In this ADR, "Spaider" is interpreted as **Framework for Documentation and Development** (working title).
<!-- spd:context -->
...
- Provide Spaider ID versioning to detect stale references
...
- `spaider.md` - Entrypoint and orchestrator
...
| Workflow | Purpose |
|----------|---------|
| `spaider.md` | Entrypoint - determines intent and dispatches to appropriate workflow |
...
#### 3. Spaider ID Versioning

Introduce version suffix to Spaider IDs to enable content change detection:
...
| `SDSL.md` | Spaider DSL (SDSL) specification |

Content requirements (what goes into each artifact type) move to the template package.

#### 5. Rebranding

Spaider is rebranded to **Framework for Documentation and Development**:
- Reflects broader scope beyond "specs"
- Maintains the "Spaider" acronym for continuity
...
- Negative: Tooling updates - `spaider` CLI needs updates for template package discovery
...
- Follow-up: Update `spaider` CLI for template package workflow dispatch
- Follow-up: Implement `rules.md` workflow for rule and template management
- Follow-up: Create migration guide for existing Spaider users
- Follow-up: Document template marker syntax and semantics
<!-- spd:list:consequences -->

**Links**:
<!-- spd:list:links -->
- Related Actors: `spd-spaider-actor-technical-lead`, `spd-spaider-actor-ai-assistant`, `spd-spaider-actor-architect`
- Related Capabilities: `spd-spaider-fr-workflow-execution`, `spd-spaider-fr-validation`, `spd-spaider-fr-adapter-config`, `spd-spaider-fr-artifact-templates`, `spd-spaider-fr-brownfield-support`
- Related Principles: `spd-spaider-principle-adapter-variability-boundary`, `spd-spaider-principle-tech-agnostic`, `spd-spaider-principle-machine-readable`, `spd-spaider-principle-traceability`
- Related ADRs: ADR-0001 (Initial Spaider Architecture), ADR-0002 (Adaptive Spaider)
<!-- spd:list:links -->
<!-- spd:##:body -->
<!-- spd:id:adr -->
