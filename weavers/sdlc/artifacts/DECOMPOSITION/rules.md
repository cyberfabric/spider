...
1. **Overall Status Checkbox** (`id:status`):
   - `[ ] p1 - spd-{hierarchy-prefix}-status-overall` — unchecked until ALL specs are implemented
   - `[x] p1 - spd-{hierarchy-prefix}-status-overall` — checked when ALL specs are `[x]`

2. **Spec Checkbox** (`id:spec`):
   - `[ ] p1 - spd-{hierarchy-prefix}-spec-{slug}` — unchecked while spec is in progress
   - `[x] p1 - spd-{hierarchy-prefix}-spec-{slug}` — checked when spec is fully implemented
...
- [ ] Generate spec IDs: `spd-{hierarchy-prefix}-spec-{slug}` (e.g., `spd-myapp-spec-user-auth`)
