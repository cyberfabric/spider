...
def test_with_mock():
    ...
    with patch("spaider.utils.context.SpaiderContext.load", return_value=mock_ctx):
...
def _bootstrap_project_root(root: Path, adapter_rel: str = "adapter") -> Path:
    ...
    (root / ".spaider-config.json").write_text(
        json.dumps({"spaiderAdapterPath": adapter_rel}),
        encoding="utf-8",
    )
...
```
skills/spaider/scripts/spaider/cli.py          90%+
skills/spaider/scripts/spaider/utils/*.py      90%+
```
...
test-coverage:
    pytest tests/ -v --cov=skills/spaider/scripts/spaider \
