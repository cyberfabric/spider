...
### Spaider Framework
Spaider (Framework for Documentation and Development) is a workflow-centered methodology framework for AI-assisted software development with design-to-code traceability.
...
### Adapter
A **project-specific adapter** in `.spaider-adapter/` that configures Spaider for a project:
...
### Artifact
A **design document** tracked by Spaider (PRD, DESIGN, DECOMPOSITION, SPEC, ADR). Each artifact:
...
- `spd-spaider-fr-1` - Functional requirement
- `spd-spaider-component-1` - Component definition
- `spd-spaider-flow-1` - Flow definition

### Spaider Marker
**Code traceability markers** linking code to design:
- `@spaider-{kind}:{id}:{phase}` - Reference marker
- `# @spaider-fr:spd-spaider-fr-1:impl` - Implementation marker

### Traceability Levels
- **FULL** - Code must have Spaider markers linking to artifact IDs
...
### CodeFile
Parsed source file with Spaider markers:
- `path` - File path
...
| `scan-ids` | Scan and list all Spaider IDs |
