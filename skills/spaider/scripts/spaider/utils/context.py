"""
Spaider Context - Global context for Spaider tooling.
...
Use SpaiderContext.load() to initialize on CLI startup.
"""
...
@dataclass
class SpaiderContext:
    """Global Spaider context with loaded metadata and templates."""
    ...
    @classmethod
    def load(cls, start_path: Optional[Path] = None) -> Optional["SpaiderContext"]:
        """Load Spaider context from adapter directory.

        Args:
            start_path: Starting path to search for adapter (default: cwd)

        Returns:
            SpaiderContext or None if adapter not found or load failed
        """
        ...
        # Load all templates for each Spaider weaver
        weavers: Dict[str, LoadedWeaver] = {}
        errors: List[str] = []

        for weaver_id, weaver in meta.weavers.items():
            if not weaver.is_spaider_format():
                continue
    ...
    def get_known_id_kinds(self) -> Set[str]:
        ...
        This is useful for parsing composite Spaider IDs.
        """
        kinds: Set[str] = set()
        for loaded_weaver in self.weavers.values():
...
_global_context: Optional[SpaiderContext] = None


def get_context() -> Optional[SpaiderContext]:
    """Get the global Spaider context."""
    return _global_context


def set_context(ctx: Optional[SpaiderContext]) -> None:
    """Set the global Spaider context."""
    global _global_context
    _global_context = ctx


def ensure_context(start_path: Optional[Path] = None) -> Optional[SpaiderContext]:
    """Ensure context is loaded, loading if necessary."""
    global _global_context
    if _global_context is None:
        _global_context = SpaiderContext.load(start_path)
    return _global_context
