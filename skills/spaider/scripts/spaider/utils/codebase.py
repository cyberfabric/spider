"""Codebase parsing/validation for Spaider traceability markers.

This module provides a deterministic, stdlib-only parser for code files with
Spaider traceability markers. Similar interface to template.py but for code.

Marker types supported:
- Scope markers: @spaider-{kind}:{id}:p{N}
- Block markers: @spaider-begin:{id}:p{N}:inst-{local} / @spaider-end:...
...
# Scope marker: @spaider-{kind}:{full-id}:p{N}
# {kind} is weaver-defined; parser accepts any lowercase slug.
_SCOPE_MARKER_RE = re.compile(
    r"@spaider-(?P<kind>[a-z][a-z0-9-]*):(?P<id>spd-[a-z0-9][a-z0-9-]+):(?:p|ph-)(?P<phase>\d+)"
)

# Block begin marker: @spaider-begin:{full-id}:ph-{N}:inst-{local}
_BLOCK_BEGIN_RE = re.compile(
    r"@spaider-begin:(?P<id>spd-[a-z0-9][a-z0-9-]+):(?:p|ph-)(?P<phase>\d+):inst-(?P<inst>[a-z0-9-]+)"
)

# Block end marker: @spaider-end:{full-id}:ph-{N}:inst-{local}
_BLOCK_END_RE = re.compile(
    r"@spaider-end:(?P<id>spd-[a-z0-9][a-z0-9-]+):(?:p|ph-)(?P<phase>\d+):inst-(?P<inst>[a-z0-9-]+)"
)
...
class ScopeMarker:
    """A scope marker like @spaider-flow:{id}:p{N}."""
    kind: str  # flow, algo, state, req, test
    id: str  # full Spaider ID
    phase: int
    line: int
...
@dataclass(frozen=True)
class BlockMarker:
    """A block marker pair @spaider-begin/end:{id}:p{N}:inst-{local}."""
    id: str  # full Spaider ID
    phase: int
...
@dataclass(frozen=True)
class CodeReference:
    """A reference to an Spaider ID found in code."""
    id: str
    line: int
...
@dataclass
class CodeFile:
    """Parsed code file with Spaider traceability markers.
    ...
    def _parse_markers(self, lines: List[str]) -> None:
        """Parse all Spaider markers from code lines."""
        ...
        for idx, line in enumerate(lines):
            ...
            for m in _BLOCK_BEGIN_RE.finditer(line):
                ...
                if key in open_blocks:
                    self._errors.append(error(
                        "marker",
                        f"Duplicate @spaider-begin without matching @spaider-end",
                        path=self.path,
                        line=line_no,
                        id=m.group("id"),
                        inst=m.group("inst"),
            ...
            for m in _BLOCK_END_RE.finditer(line):
                ...
                if key not in open_blocks:
                    self._errors.append(error(
                        "marker",
                        f"@spaider-end without matching @spaider-begin",
                        path=self.path,
                        line=line_no,
                        id=m.group("id"),
                        inst=m.group("inst"),
                    ...
                    if not content or all(not ln.strip() for ln in content):
                        self._errors.append(error(
                            "marker",
                            "Empty block (no code between @spaider-begin and @spaider-end)",
                            path=self.path,
                            line=start_line,
                            id=spd,
                            inst=inst,
        ...
        for key, (start_line, spd, phase, inst) in open_blocks.items():
            self._errors.append(error(
                "marker",
                "@spaider-begin without matching @spaider-end",
                path=self.path,
                line=start_line,
                id=spd,
                inst=inst,
    ...
    def list_ids(self) -> List[str]:
        """List all unique Spaider IDs referenced in this code file."""
        ids: Set[str] = set()
    ...
    def get(self, id_value: str) -> Optional[str]:
        """Get the code content associated with an Spaider ID.
    ...
    if traceability == "DOCS-ONLY":
        ...
        for cf in code_files:
            if cf.scope_markers or cf.block_markers:
                errors.append(error(
                    "traceability",
                    "Spaider markers found in code but traceability is DOCS-ONLY",
                    path=cf.path,
                    line=1,
                ))
        return {"errors": errors, "warnings": warnings}
