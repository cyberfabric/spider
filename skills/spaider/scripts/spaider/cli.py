...
Command-line interface for the Spaider validation tool.
"""
        ...
        "agents": {
            "windsurf": {
                "workflows": {
                    ...
                    "workflow_command_prefix": "spaider-",
                ...
                "skills": {
                    ...
                    "outputs": [
                        {
                            "path": ".windsurf/skills/spaider/SKILL.md",
                            "template": [
                                "---",
                                "name: {name}",
                        ...
                        {
                            "path": ".windsurf/workflows/spaider.md",
                            "template": [
                                "# /spaider",
                                "",
                                "{custom_content}",
            ...
            "cursor": {
                ...
                "skills": {
                    ...
                    "outputs": [
                        {
                            "path": ".cursor/weavers/spaider.mdc",
                            "template": [
                                "---",
                                "description: {description}",
                                "alwaysApply: true",
                        ...
                        {
                            "path": ".cursor/commands/spaider.md",
                            "template": [
                                "# /spaider",
                                "",
                                "{custom_content}",
            ...
            "claude": {
                ...
                "skills": {
                    ...
                    "outputs": [
                        {
                            "path": ".claude/commands/spaider.md",
                            "template": [
                                "---",
                                "description: {description}",
                                "---",
                        ...
                        {
                            "path": ".claude/skills/spaider/SKILL.md",
                            "template": [
                                "---",
                                "name: spaider",
                                "description: {description}",
                        ...
                        {
                            "path": ".claude/skills/spaider-adapter/SKILL.md",
                        ...
                        {
                            "path": ".claude/skills/spaider-generate/SKILL.md",
                        ...
                        {
                            "path": ".claude/skills/spaider-analyze/SKILL.md",
            ...
            "copilot": {
                ...
                "skills": {
                    ...
                    "outputs": [
                        ...
                        {
                            "path": ".github/prompts/spaider.prompt.md",
                            "template": [
                                "---",
                                "name: {name}",
                                "description: {description}",
            ...
            "openai": {
                "skills": {
                    ...
                    "outputs": [
                        {
                            "path": ".agents/skills/spaider/SKILL.md",
                            "template": [
                                "---",
                                "name: {name}",
                                "description: {description}",
...
def _cmd_self_check(argv: List[str]) -> int:
    ...
    p.add_argument("--weaver", "--rule", dest="weaver", help="Specific weaver ID to check (e.g., spaider-sdlc)")
...
def _cmd_agents(argv: List[str]) -> int:
    ...
    p.add_argument("--spaider-root", default=None, help="Explicit Spaider core root (optional override)")
    p.add_argument("--config", default=None, help="Path to unified agents config JSON (default: spaider-agents.json in project root)")
    ...
    if project_root is None:
        print(json.dumps({
            "status": "NOT_FOUND",
            "message": "No project root found (no .git or .spaider-config.json)",
            "searched_from": start_path.as_posix(),
        }, indent=2, ensure_ascii=False))
        return 1

    spaider_root = Path(args.spaider_root).resolve() if args.spaider_root else None
    if spaider_root is None:
        spaider_root = (Path(__file__).resolve().parents[4])
        if not ((spaider_root / "AGENTS.md").exists() and (spaider_root / "workflows").is_dir()):
            spaider_root = Path(__file__).resolve().parents[6]

    cfg_path = Path(args.config).resolve() if args.config else (project_root / "spaider-agents.json")
    ...
    if isinstance(workflows_cfg, dict) and workflows_cfg:
        ...
        prefix = workflows_cfg.get("workflow_command_prefix", "spaider-")
        ...
        if not isinstance(workflow_dir_rel, str) or not workflow_dir_rel.strip():
            ...
            spaider_workflow_files = _list_workflow_files(spaider_root)
            spaider_workflow_names = [Path(p).stem for p in spaider_workflow_files]

            desired: Dict[str, Dict[str, str]] = {}
            for wf_name in spaider_workflow_names:
                command = "spaider" if wf_name == "spaider" else f"{prefix}{wf_name}"
                filename = filename_fmt.format(command=command, workflow_name=wf_name)
                desired_path = (workflow_dir / filename).resolve()
                target_workflow_path = (spaider_root / "workflows" / f"{wf_name}.md").resolve()

                # If a skill output (e.g., /spaider) already owns this path, do not generate a workflow proxy.
                ...
                source_description = fm.get("description", f"Proxy to Spaider workflow {wf_name}")
            ...
            for pth in existing_files:
                ...
                if p_str in desired_paths:
                    continue
                if not pth.name.startswith(prefix) and not pth.name.startswith("spaider-"):
                    continue
                try:
                    txt = pth.read_text(encoding="utf-8")
                except Exception:
                ...
                try:
                    expected.relative_to(spaider_root / "workflows")
                except ValueError:
                    continue
                if expected.exists():
                    continue
    ...
    if isinstance(skills_cfg, dict) and skills_cfg:
        outputs = skills_cfg.get("outputs")
        skill_name = skills_cfg.get("skill_name", "spaider")
        ...
        if outputs is not None:
            if not isinstance(outputs, list) or not all(isinstance(x, dict) for x in outputs):
                ...
                # Spaider skill source is always located relative to this script.
                # `.../skills/spaider/scripts/spaider/cli.py` -> `.../skills/spaider/SKILL.md`
                ...
                if not target_skill_abs.is_file():
                    skills_result["errors"].append(
                        "Spaider skill source not found (expected: " + target_skill_abs.as_posix() + ")"
                    )
                ...
                skill_source_description = skill_fm.get("description", "Proxy to Spaider core skill instructions")
                ...
                for idx, out_cfg in enumerate(outputs):
                    ...
                    if custom_target:
                        target_abs = (spaider_root / custom_target).resolve()
    ...
    print(json.dumps({
        ...
        "agent": agent,
        "project_root": project_root.as_posix(),
        "spaider_root": spaider_root.as_posix(),
        "config_path": cfg_path.as_posix(),
...
def _default_project_config(spaider_core_path: str, spaider_adapter_path: str) -> dict:
    return {
        "spaiderCorePath": spaider_core_path,
        "spaiderAdapterPath": spaider_adapter_path,
        "codeScanning": {
            "fileExtensions": [
                ".py",
                ".md",
...
def _list_workflow_files(spaider_root: Path) -> List[str]:
    workflows_dir = (spaider_root / "workflows").resolve()
...
def _cmd_init(argv: List[str]) -> int:
    p = argparse.ArgumentParser(prog="init", description="Initialize Spaider config and minimal adapter")
    p.add_argument("--project-root", default=None, help="Project root directory to create .spaider-config.json in")
    p.add_argument("--spaider-root", default=None, help="Explicit Spaider core root (optional override)")
    p.add_argument("--adapter-path", default=None, help="Adapter directory path relative to project root (default: .spaider-adapter)")
    ...
    if args.project_root is None and not args.yes:
        raw_root = _prompt_path("Where should I create .spaider-config.json?", default_project_root.as_posix())
    ...
    config_path = (project_root / ".spaider-config.json").resolve()
    existing_cfg = _load_json_file(config_path) if config_path.is_file() else None

    default_adapter_path = ".spaider-adapter"
    if args.adapter_path is None and isinstance(existing_cfg, dict):
        existing_adapter_path = existing_cfg.get("spaiderAdapterPath")
    ...
    if args.adapter_path is None and not args.yes:
        adapter_rel = _prompt_path("Where should I create the Spaider adapter directory (relative to project root)?", default_adapter_path)
    else:
    ...
    core_rel = _safe_relpath_from_dir(spaider_root, project_root)
    extends_target = (spaider_root / "AGENTS.md").resolve()
    ...
    weaver_id = "spaider-sdlc"
    artifacts_when = f"ALWAYS open and follow `artifacts.json` WHEN Spaider uses weaver `{weaver_id}` for artifact kinds: PRD, DESIGN, DECOMPOSITION, ADR, SPEC OR codebase"
    # Use {spaider_path} variable instead of relative paths for Spaider core references
    desired_agents = "\n".join([
        f"# Spaider Adapter: {project_name}",
        "",
    ...
    if config_existed_before and not config_path.is_file():
        ...
        if not isinstance(existing, dict):
            ...
            existing_core = merged.get("spaiderCorePath")
            if not (isinstance(existing_core, str) and existing_core.strip()):
                merged["spaiderCorePath"] = core_rel
                changed = True

            existing_adapter = merged.get("spaiderAdapterPath")
            if not (isinstance(existing_adapter, str) and existing_adapter.strip()):
                merged["spaiderAdapterPath"] = adapter_rel
                changed = True
            ...
            elif merged.get("spaiderCorePath") != core_rel or merged.get("spaiderAdapterPath") != adapter_rel:
...
def _cmd_validate(argv: List[str]) -> int:
    """Validate Spaider artifacts and code traceability.
    ...
    p = argparse.ArgumentParser(
        prog="validate",
        description="Validate Spaider artifacts and code traceability (structure + cross-refs + traceability)",
    )
    p.add_argument("--artifact", default=None, help="Path to specific Spaider artifact (if omitted, validates all registered Spaider artifacts)")
    ...
    if args.artifact:
        ...
        if rel_path:
            ...
            if result:
                ...
                if pkg and pkg.is_spaider_format():
        ...
        if not artifacts_to_validate:
            print(json.dumps({"status": "ERROR", "message": f"Artifact not in Spaider registry: {args.artifact}"}, indent=None, ensure_ascii=False))
            return 1
    else:
        # Validate all Spaider artifacts
        for artifact_meta, system_node in meta.iter_all_artifacts():
            pkg = meta.get_weaver(system_node.weaver)
            if not pkg or not pkg.is_spaider_format():
                continue
            template_path_str = pkg.get_template_path(artifact_meta.kind)
    ...
    if not artifacts_to_validate:
        print(json.dumps({"status": "ERROR", "message": "No Spaider artifacts found in registry"}, indent=None, ensure_ascii=False))
        return 1
    ...
    for artifact_path, template_path, artifact_type, traceability in artifacts_to_validate:
        ...
        from .utils.document import file_has_spaider_markers

        is_markerless = not file_has_spaider_markers(artifact_path)
    ...
    # Cross-reference validation - load ALL Spaider artifacts for context
    ...
    for artifact_path, _template_path, artifact_kind, traceability in artifacts_to_validate:
        if traceability != "FULL":
            continue
        if file_has_spaider_markers(artifact_path):
            continue
...
def _cmd_list_ids(argv: List[str]) -> int:
    """List Spaider IDs from artifacts using template-based parsing.

    If no artifact is specified, scans all Spaider-format artifacts from the adapter registry.
    """
    p = argparse.ArgumentParser(prog="list-ids")
    p.add_argument("--artifact", default=None, help="Path to Spaider artifact file (if omitted, scans all registered Spaider artifacts)")
    ...
    p.add_argument("--include-code", action="store_true", help="Also scan code files for Spaider marker references")
    args = p.parse_args(argv)
    ...
    if args.artifact:
        ...
        from .utils.context import SpaiderContext
        ctx = SpaiderContext.load(artifact_path.parent)
        if not ctx:
        ...
        if not artifacts_to_scan:
            print(json.dumps({"status": "ERROR", "message": "No Spaider-format artifacts found in registry."}, indent=None, ensure_ascii=False))
            return 1
...
def _cmd_list_id_kinds(argv: List[str]) -> int:
    ...
    """
    p = argparse.ArgumentParser(prog="list-id-kinds", description="List ID kinds found in Spaider artifacts")
    p.add_argument("--artifact", default=None, help="Scan specific artifact (if omitted, scans all registered Spaider artifacts)")
    args = p.parse_args(argv)
...
def _cmd_get_content(argv: List[str]) -> int:
    """Get content block for a specific Spaider ID using template-based parsing."""
    p = argparse.ArgumentParser(prog="get-content", description="Get content block for a specific Spaider ID")
    p.add_argument("--artifact", default=None, help="Path to Spaider artifact file")
    p.add_argument("--code", default=None, help="Path to code file (alternative to --artifact)")
    p.add_argument("--id", required=True, help="Spaider ID to retrieve content for")
    ...
    # Load SpaiderContext from artifact's location
    from .utils.context import SpaiderContext
    ctx = SpaiderContext.load(artifact_path.parent)
    if not ctx:
...
def _cmd_where_defined(argv: List[str]) -> int:
    """Find where an Spaider ID is defined using template-based parsing."""
    p = argparse.ArgumentParser(prog="where-defined", description="Find where an Spaider ID is defined")
    p.add_argument("--id", required=True, help="Spaider ID to find definition for")
...
def _cmd_where_used(argv: List[str]) -> int:
    """Find all references to an Spaider ID using template-based parsing."""
    p = argparse.ArgumentParser(prog="where-used", description="Find all references to an Spaider ID")
    p.add_argument("--id", required=True, help="Spaider ID to find references for")
    p.add_argument("--artifact", default=None, help="Limit search to specific artifact (optional)")
...
def _cmd_validate_weavers(argv: List[str]) -> int:
    """Validate Spaider weaver packages and template files.

    Checks that:
    - Weavers are properly configured in artifacts.json
    - Templates have valid spaider-template frontmatter (kind, version)
    - All Spaider markers are properly paired (open/close)
    - Marker types and attributes are valid
    """
    p = argparse.ArgumentParser(prog="validate-weavers", description="Validate Spaider weaver packages and template files")
    ...
    if args.template:
        ...
        # Collect unique template paths from all Spaider artifacts
        ...
        if not templates_to_validate:
            print(json.dumps({"status": "ERROR", "message": "No Spaider templates found in registry"}, indent=None, ensure_ascii=False))
            return 1
...
def _cmd_adapter_info(argv: List[str]) -> int:
    """
    Discover and display Spaider adapter information.
    Shows adapter location, project name, and available specs.
    """
    p = argparse.ArgumentParser(prog="adapter-info", description="Discover Spaider adapter configuration")
    p.add_argument("--root", default=".", help="Project root to search from (default: current directory)")
    p.add_argument("--spaider-root", default=None, help="Spaider core location (if agent knows it)")
    args = p.parse_args(argv)
    
    start_path = Path(args.root).resolve()
    spaider_root_path = Path(args.spaider_root).resolve() if args.spaider_root else None
        ...
        print(json.dumps(
            {
                ...
                "message": "No project root found (no .git or .spaider-config.json)",
                "searched_from": start_path.as_posix(),
                "hint": "Create .spaider-config.json in project root to configure Spaider",
            },
            indent=2,
            ensure_ascii=False,
        ))
    ...
    # Find adapter
    adapter_dir = find_adapter_directory(start_path, spaider_root=spaider_root_path)
    if adapter_dir is None:
                ...
                print(json.dumps(
                    {
                        ...
                        "hint": "Check .spaider-config.json spaiderAdapterPath points to valid directory with AGENTS.md",
                    },
                    indent=2,
                    ensure_ascii=False,
                ))
        ...
        print(json.dumps(
            {
                "status": "NOT_FOUND",
                "message": "No .spaider-adapter found in project (searched recursively up to 5 levels deep)",
                "project_root": project_root.as_posix(),
                "hint": "Create .spaider-config.json with spaiderAdapterPath or run adapter-bootstrap workflow",
            },
            indent=2,
            ensure_ascii=False,
        ))
    ...
    # Check if .spaider-config.json exists
    config_file = project_root / ".spaider-config.json"
    config["has_config"] = config_file.exists()
    if not config_file.exists():
        config["config_hint"] = f"Create .spaider-config.json with: {{\"spaiderAdapterPath\": \"{relative_path}\"}}"
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    # Load global Spaider context on startup (templates, systems, etc.)
    # Always reload context based on current working directory (no caching)
    from .utils.context import SpaiderContext, set_context
    ctx = SpaiderContext.load()
    set_context(ctx)
    ...
    if argv_list and argv_list[0] in ("-h", "--help"):
        print("usage: spaider <command> [options]")
        print()
        print("Spaider CLI - artifact validation and traceability tool")
        print()
        ...
        print()
        print("Run 'spaider <command> --help' for command-specific options.")
        return 0
