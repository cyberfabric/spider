...
class ParsedSpiderId:
    """Result of parsing an Spider ID."""
    system: str
    kind: str
    slug: str
...
class Template:
    ...
    def parse_blocks(lines: List[str]) -> Tuple[List[TemplateBlock], List[Dict[str, object]]]:
        """Parse paired spider markers into TemplateBlock objects with spans and attrs."""
    ...
    def load(self) -> None:
        ...
        if lines and lines[0].strip() == "---":
            for i, ln in enumerate(lines[1:], start=2):
                if ln.strip() == "---":
                    break
                if "spider-template:" in ln:
                    self._errors.append(Template.error(
                        "structure",
                        "Artifact contains template frontmatter (spider-template:) - this belongs only in template files, not artifacts",
                        path=self.path,
                        line=i,
                    ))
                    break
...
def cross_validate_artifacts(
    ...
    # Helper to extract kind from Spider ID (first segment after system)
    def _extract_kind_from_id(spd: str) -> Optional[str]:
        """Extract the kind segment from an Spider ID."""
        if not spd.lower().startswith("spd-"):
            return None
...
def parse_spd(
    ...
    known_kinds: Optional[Iterable[str]] = None,
) -> Optional[ParsedSpiderId]:
    """Parse an Spider ID and extract system, kind, slug, and optional prefix_id.
    ...
    if system is None:
        return None  # unknown system — not a recognized Spider ID
    ...
    if first_kind.lower() == expected_kind.lower():
        ...
        return ParsedSpiderId(system=system, kind=expected_kind, slug=slug, prefix_id=None)
    ...
    return ParsedSpiderId(system=system, kind=expected_kind, slug=slug, prefix_id=left)
