"""
FDD Validator - Language Configuration
...
def build_fdd_begin_regex(lang_config: LanguageConfig) -> re.Pattern:
    """Build fdd-begin regex pattern using language config."""
    comment_pattern = lang_config.build_comment_pattern()
    return re.compile(rf"^\s*{comment_pattern}\s*(?:!no-fdd\s+)?fdd-begin\s+([^\s]+)")


def build_fdd_end_regex(lang_config: LanguageConfig) -> re.Pattern:
    """Build fdd-end regex pattern using language config."""
    comment_pattern = lang_config.build_comment_pattern()
    return re.compile(rf"^\s*{comment_pattern}\s*(?:!no-fdd\s+)?fdd-end\s+([^\s]+)")


def build_no_fdd_begin_regex(lang_config: LanguageConfig) -> re.Pattern:
    """Build !no-fdd-begin regex pattern using language config."""
    comment_pattern = lang_config.build_comment_pattern()
    return re.compile(rf"^\s*{comment_pattern}.*!no-fdd-begin")


def build_no_fdd_end_regex(lang_config: LanguageConfig) -> re.Pattern:
    """Build !no-fdd-end regex pattern using language config."""
    comment_pattern = lang_config.build_comment_pattern()
    return re.compile(rf"^\s*{comment_pattern}.*!no-fdd-end")
